...
def test_client_uses_json_stdin_and_clean_stdout(tmp_path: Path) -> None:
    ...
    executable.write_text(
        "#!/usr/bin/env python3\nimport json,sys\nrequest=json.loads(sys.stdin.read())\nassert request['capability'] == 'environment.describe'\nprint(json.dumps({'protocol_version':'1.0','request_id':request['request_id'],'ok':True,'result':{'capabilities':[{'id':'context.open'}]}},separators=(',',':')))\nprint('diagnostic', file=sys.stderr)\n",
        encoding="utf-8",
    )
    executable.chmod(0o755)
    client = GridctlClient(executable=executable, workspace=tmp_path, timeout_seconds=5)

    result = client.call("environment.describe", {})
