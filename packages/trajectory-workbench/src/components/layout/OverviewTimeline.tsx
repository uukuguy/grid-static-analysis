    ...
    <svg viewBox={`0 0 ${Math.max(problems.length, 1) * 120} 54`} role="img" aria-label="Run turn overview">
      {problems.map((problem, index) => {
        ...
        return <g key={problem.id}>
          <rect x={x} y="8" width="108" height="34" rx="6" className={active ? 'timeline-turn active' : 'timeline-turn'}
            onClick={() => { onSelectTurn(problem.turn_id); onFocusRange({ startSequence: Math.min(...problem.source_sequences), endSequence: Math.max(...problem.source_sequences) }); }} />
